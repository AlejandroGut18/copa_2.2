...
// Alerta de inactividad luego de 10 minutos
<script>
    const tiempoMaximo = 600; // segundos
    const tiempoAviso = 20; // segundos antes de expirar
    let contador = tiempoMaximo;

    const reiniciarContador = () => {
        contador = tiempoMaximo;
    };
    ...
    // Temporizador para controlar el tiempo de inactividad
    setInterval(() => {
        contador--;

        // Mostrar alerta cuando queden 20 segundos
        if (contador === tiempoAviso) {
            Swal.fire({
                ...
                timer: tiempoAviso * 1000,
                timerProgressBar: true
            }).then((result) => {
                if (result.isConfirmed) {
                    // Enviar ping al servidor para reiniciar actividad
                    fetch('<?php echo base_url; ?>Config/ping.php')
                        .then(() => {
                            reiniciarContador();
                        });
                } else {
                    // Redirige al logout o destruye la sesión
                    window.location.href = '<?php echo base_url; ?>Usuarios/salir';
                }
            });
        }

        // Si llega a cero sin respuesta
        if (contador <= 0) {
            window.location.href = '<?php echo base_url; ?>Usuarios/salir';
        }
    }, 1000);
