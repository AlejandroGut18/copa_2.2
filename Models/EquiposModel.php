...
class EquiposModel extends Query
{
    ...
    public function insertarEquipo($nombre, $delegado, $status_id, $genero, $tipo)
    {
        ...
        if (empty($existe)) {
            $query = "INSERT INTO equipos(nombre, delegado_equipo, status_id, genero, tipo_equipo) VALUES (?, ?, ?, ?, ?)";
            $datos = array($nombre, $delegado, $status_id, $genero, $tipo);
    ...
    public function actualizarEquipo($nombre, $delegado, $status_id, $genero, $id, $tipo)
    {
        $query = "UPDATE equipos SET nombre = ?, delegado_equipo = ?, status_id = ?, genero = ?, tipo_equipo = ? WHERE id = ?";
        $datos = array($nombre, $delegado, $status_id, $genero, $tipo, $id);
        $data = $this->save($query, $datos);
    ...
    public function getEquiposNoInscritos($genero, $torneo_id)
    {
        $sql = "SELECT e.id, e.nombre
            FROM equipos e
            LEFT JOIN inscripciones_equipos i 
                ON e.id = i.equipo_id 
                AND i.torneo_id = ? 
